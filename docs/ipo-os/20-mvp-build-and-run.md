...
src/lib/cdp/                 # the core (no external deps; pure TS + JSON)
  ...
  creditScore.ts             # the moat: credit-from-day-one score + explainable factors
  ...
  index.ts                   # single entry point (used by the page, API, and MCP server)
src/app/capital/page.tsx     # the dashboard (credit score, offers, alerts, "name a firm")
src/app/capital/FirmSelector.tsx
src/app/api/cdp/*            # state / checklist / providers JSON endpoints
mcp/server.ts                # MCP server (one server, many agents) — npm run mcp
scripts/cdp_smoke.ts         # smoke test (no install needed)
```

## Run it locally
```bash
npm install
npm run dev          # open http://localhost:3000/capital
```
The `/capital` dashboard reads `src/data/diligence/{catalog.json,providers.json}` directly —
**no database needed** for the demo (you do not need to run the diligence loader).
